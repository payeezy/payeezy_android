...
import com.firstdata.firstapi.client.domain.v2.ThreeDS;
...
import com.sample.timeoutreversal.MainActivity;
...
Random rand=new Random();
	...
	public void randomnumbergeneration()
	{
		...
		TransactionDataProvider.reversal_id=null;
		TransactionDataProvider.reversal_id="Re-txn"+"-"+newrandom;

	}
	@Override
	protected String doInBackground(String... uri) {
		/***************************TimeOutReversal**********************/
		if (uri[0].toLowerCase().equalsIgnoreCase("toravsauthorisereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.AVS, uri);

			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);


			return "toravsauthorisereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("toravspurchasereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.AVS, uri);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "toravspurchasereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("toravsauthorisecapturereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.AVS, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.CAPTURE);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);


			return "toravsauthorisecapturereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("toravspurchaserefundreversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.AVS, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.REFUND);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "toravspurchaserefundreversal";
		}

		/*****CVV**************/
		if (uri[0].toLowerCase().equalsIgnoreCase("torcvvauthorisereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.CVV, uri);
				CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);


			return "torcvvauthorisereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("torcvvpurchasereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.CVV, uri);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "torcvvpurchasereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("torcvvauthorisecapturereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.CVV, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.CAPTURE);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "torcvvauthorisecapturereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("torcvvpurchaserefundreversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.CVV, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.REFUND);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "torcvvpurchaserefundreversal";
		}

		/*********3DS**********/
		if (uri[0].toLowerCase().equalsIgnoreCase("tor3dsauthorisereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.THREE_DS, uri);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "tor3dsauthorisereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("tor3dspurchasereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.THREE_DS, uri);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "tor3dspurchasereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("tor3dsauthorisecapturereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.THREE_DS, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.CAPTURE);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "tor3dsauthorisecapturereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("tor3dspurchaserefundreversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.THREE_DS, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.REFUND);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);
			return "tor3dspurchaserefundreversal";
		}

		/*********SoftDescriptor**********/
		if (uri[0].toLowerCase().equalsIgnoreCase("torsdauthorisereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.SOFT_DESCRIPTOR, uri);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);
			return "torsdauthorisereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("torsdpurchasereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.SOFT_DESCRIPTOR, uri);
CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "torsdpurchasereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("torsdauthorisecapturereversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.AUTHORISE, CardFeature.SOFT_DESCRIPTOR, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.CAPTURE);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);
			return "torsdauthorisecapturereversal";
		}
		if (uri[0].toLowerCase().equalsIgnoreCase("torsdpurchaserefundreversal")) {
			CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary.PURCHASE, CardFeature.SOFT_DESCRIPTOR, uri);
			CallTORCaptureVoidRefund(TransactionTypePrimarySecondary.REFUND);
			CallCommonTimeOutReversal(TransactionDataProvider.secreversal_id);
			CallCommonTimeOutReversal(TransactionDataProvider.reversal_id);

			return "torsdpurchaserefundreversal";
		}
/*************************DCC*************************************/
...
//DCC-CVV
	...
	//Initialise method: for initialising the CardType, appId,SecretKey, token and Url for DCC
	...
	//Initialise method: for initialising the CardType, appId,SecretKey, token and Url for TimeOutReversal
	private void InitialiseTOR(){
		clientHelper.setAppId(TransactionDataProvider.appIdTOR);
		clientHelper.setSecuredSecret(TransactionDataProvider.secIDTOR);
		clientHelper.setToken(TransactionDataProvider.tokenTOR);
		clientHelper.setUrl(TransactionDataProvider.urlTOR);

	}
/*****************TimeOutReversal***************/

private void CallCommonTimeOutReversal(String reversalId)
{
	try {
		InitialiseTOR();
		...
		request.setTransactionType(TransactionType.VOID.name().toLowerCase());
		request.setReversal_id(reversalId);

		Object responseObject = clientHelper.doPrimaryTransactionGD(request);
	...
	private void CallTORCVVAVS3DSSoftDesc(TransactionTypePrimarySecondary transactionType, CardFeature cardFeature,String[] uri){
		try {
			...
			if(cardFeature!=CardFeature.THREE_DS)
			 trans = getPrimaryTransactionTOR(uri);
			...
			switch(cardFeature)
			{
				...
				case AVS:
					trans=	CallTORAVSObjInit(trans, uri);
					break;
				case SOFT_DESCRIPTOR:
					trans= CallTORSoftDescObjInit(trans, uri);
					break;
				case THREE_DS:
					trans.setAmount(uri[1]);
					trans.setPaymentMethod(uri[3]);
					trans.setCurrency(uri[4]);

					trans=CallTOR3DSObjInit(trans,uri);
							break;
				default:
					break;
		...
		trans.setReversal_id(TransactionDataProvider.reversal_id);
	...
	/**For setting the CVV object [base payload request] for TimeOutReversal***********/
	private TransactionRequest getPrimaryTransactionTOR(String[]uri ) {
		TransactionRequest request = new TransactionRequest();
		...
		card.setType(uri[5]);
		card.setName(uri[6]);
		card.setNumber(uri[7]);
		card.setExpiryDt(uri[8]);
	...
	private TransactionRequest CallTORAVSObjInit(TransactionRequest trans, String[] uri)
	{
	...
	private TransactionRequest CallTORSoftDescObjInit(TransactionRequest trans, String[] uri)
	{
		SoftDescriptor softDescriptor=new SoftDescriptor();
		softDescriptor.setDba_name(uri[10]);
		softDescriptor.setStreet(uri[11]);
		softDescriptor.setRegion(uri[12]);
		softDescriptor.setMid(uri[13]);
		softDescriptor.setMcc(uri[14]);
		softDescriptor.setPostalCode(uri[15]);
		softDescriptor.setCountryCode(uri[16]);
		softDescriptor.setMerchantContactInfo(uri[17]);

		trans.setDescriptor(softDescriptor);
		return trans;
	}

 private TransactionRequest CallTOR3DSObjInit(TransactionRequest trans, String[] uri) {

	 ThreeDomainSecureToken threeDomainSecureToken=new ThreeDomainSecureToken();
	 threeDomainSecureToken.setType(uri[5]);
	 threeDomainSecureToken.setCardholder_name(uri[6]);
	 threeDomainSecureToken.setCard_number(uri[7]);
	 threeDomainSecureToken.setExp_date(uri[8]);
	 threeDomainSecureToken.setCvv(uri[9]);
	 threeDomainSecureToken.setCavv(uri[10]);
	 threeDomainSecureToken.setXid(uri[11]);

	 trans.setThreeDomainSecureToken(threeDomainSecureToken);
	 return trans;
 }

	//TimeOutReversal-Secondary Transaction--void, capture, refund
	private void CallTORCaptureVoidRefund(TransactionTypePrimarySecondary transactionType) {
		try {
			InitialiseTOR();

			TransactionRequest trans = getSecondaryTransactionGD();
			trans.setPaymentMethod("credit_card");
			TransactionDataProvider.secreversal_id=null;
			TransactionDataProvider.secreversal_id="Re-txn"+"-"+TransactionResponse.getCorrelationID();
			trans.setReversal_id(TransactionDataProvider.secreversal_id);
